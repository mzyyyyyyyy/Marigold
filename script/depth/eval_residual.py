"""
Residual analysis for FM Refiner.

Computes:
  residual_pred = H_refined - H_coarse
...
def _make_residual_figure(samples: list) -> plt.Figure:
    ...
    for row, s in enumerate(samples):
        ...
        r_gt   = s["residual_gt"]
        abs_max = max(np.abs(r_pred).max(), np.abs(r_gt).max(), 1e-6)
...
def run_residual_eval(
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Residual eval"):
    ...
    return pearson_r, pearson_p, fig


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FM Refiner – Residual Analysis")
    ...
    parser.add_argument("--n_steps",  type=int, default=None,
                        help="Euler steps (overrides config).")
    parser.add_argument("--n_vis",    type=int, default=20,
                        help="Number of samples to visualize.")
    parser.add_argument("--save_dir", type=str, default="output/fm_refiner_v3/residual_analysis",
                        help="Directory for output files (defaults to ckpt directory).")
    ...
    with open(results_path, "w") as f:
        json.dump({"pearson_r": float(pearson_r), "pearson_p": float(pearson_p)}, f, indent=2)
